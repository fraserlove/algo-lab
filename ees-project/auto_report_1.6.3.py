...
def app():
    ...
    def root_screen(root):
        ...
        def main(root):
            ...
            def create_pdf(page, id, time, name, manufacturer_name, machine_model, machine_id, customer_name, contact_name, location, job_type, customer_charge, description):
                if locked == False:
                    try:
                        pdf = canvas.Canvas("report.pdf")
                        pdf.setFont("Helvetica-Bold", 30, leading=None)
                        pdf.drawCentredString(300, 750, "Report")
                        pdf.setFont("Helvetica", 14, leading=None)
                        pdf.drawCentredString(300, 700, ("Job Number: {}".format(id)))
                        pdf.drawCentredString(300, 670, ("Time Sent: {}".format(time)))
                        pdf.drawCentredString(300, 640, ("Engineer Name: {}".format(name)))
                        pdf.drawCentredString(300, 610, ("Manufacturer Name: {}".format( manufacturer_name)))
                        pdf.drawCentredString(300, 580, ("Machine Model: {}".format(machine_model)))
                        pdf.drawCentredString(300, 550, ("Machine ID: {}".format(machine_id)))
                        pdf.drawCentredString(300, 520, ("Customer Name: {}".format(customer_name)))
                        pdf.drawCentredString(300, 490, ("Contact Name: {}".format(contact_name)))
                        pdf.drawCentredString(300, 460, ("Location: {}".format(location)))
                        pdf.drawCentredString(300, 430, ("Job Type: {}".format(job_type)))
                        pdf.drawCentredString(300, 400, ("Customer Charge: {}".format(customer_charge)))
                        pdf.drawCentredString(300, 370, ("Description: {}".format(description)))
                        logo = "data/logo.jpg"
