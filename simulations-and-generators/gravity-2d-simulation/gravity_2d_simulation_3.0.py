...
initial_objects = 400   # Number of initial objects in the simulation
object_list = []
...
def update_pos(object_list):
    for object in object_list:
        ...
        if object.x_velocity + (x_acc * time_step) < max_v and object.x_velocity + (x_acc * time_step) > -max_v:
            object.x_velocity += x_acc * time_step
        elif object.x_velocity + (x_acc * time_step) > max_v:
            object.x_velocity = max_v
        elif object.x_velocity + (x_acc * time_step) < -max_v:
            object.x_velocity = -max_v
        if object.y_velocity + (y_acc * time_step) < max_v and object.y_velocity + (y_acc * time_step) > -max_v:
